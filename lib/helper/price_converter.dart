...
class PriceConverter {
  static String convertPrice(BuildContext context, double? price,
    ...
    return '${_inRight ? '' : Provider.of<SplashProvider>(context, listen: false).myCurrency!.symbol}'
        // * Provider.of<SplashProvider>(context, listen: false).myCurrency.exchangeRate * (1 / Provider.of<SplashProvider>(context, listen: false).usdCurrency.exchangeRate)).toStringAsFixed(Provider.of<SplashProvider>(context, listen: false).configModel.decimalPointSetting ?? 1).replaceAllMapped(RegExp(r'(\d{1,3})(?=(\d{3})+(?!\d))'), (Match m) => '${m[1]},')
  ...
  static double calculation(
    ...
    if (type == 'amount' || type == 'flat') {
      calculatedAmount = discount * (quantity != null ? quantity : 1);
    } else if (type == 'percent' || type == 'percentage') {
      calculatedAmount =
          (discount / 100) * (amount * (quantity != null ? quantity : 1));
    }
