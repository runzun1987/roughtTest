...
class LocationProvider with ChangeNotifier {
  ...
  set selectedTown(String? selectedTown) {
  ...
  void getCurrentLocation(BuildContext context, bool fromAddress,
    ...
    try {
      List<String?> _address = await getAddressFromGeocode(
      ...
      _myPlaceMark = Placemark(
          name: _address[0], locality: '', postalCode: '', country: '');
    } catch (e) {
      List<String?> _address = await getAddressFromGeocode(
          LatLng(_myPosition.latitude, _myPosition.longitude), context);
      _myPlaceMark = Placemark(
  ...
  void updatePosition(CameraPosition position, bool fromAddress, List? address,
    ...
    if (_updateAddAddressData) {
      ...
      print('dddd');
      _loading = true;
      print('truuuuuuuuuuuueee');

      notifyListeners();
      print(
          'tryyyyyyyyyyyyyyyyyyyyyyyyyyyyyyyyyyyyyyyyyyyyyyyyyyyyyyyyyyyyyyyyyyyyyyyyyyyyyyyyyyy');
      try {
        if (fromAddress) {
          print('frommmmaddddreesssssws');
          _position = Position(
          ...
          print('ddasdfasdfasdfasdfasdf');
        } else {
          print('adddsssssssssssssssssss');
        ...
        if (_changeAddress) {
          ...
          List<String?> _addresss = await getAddressFromGeocode(
              LatLng(position.target.latitude, position.target.longitude),
              context);
          _selectedTown = _addresss[1];
          _country = _addresss[4];

          fromAddress
              ? _address = Placemark(name: _addresss[0])
              : _pickAddress = Placemark(name: _addresss[0]);

          if (address != null) {
            _locationController.text = address[0];
            _selectedTown = address[1];
  ...
  void setAddAddressData(context) {
    _position = _pickPosition;
    // _address = _pickAddress;

    CameraPosition cameraPosition = CameraPosition(
        target: LatLng(_pickPosition.latitude, _pickPosition.longitude),
        zoom: 14);
  ...
  Future<List<String?>> getAddressFromGeocode(
    ...
    if (response.response!.statusCode == 200 &&
      ...
      response.response!.data['results'][0]['address_components'].forEach((a) {
        if (a["types"][0] == "route") {
          street = a['long_name'];
        }
        if (a['types'][0] == 'political') {
          street2 = a['long_name'];
        }
      ...
      } else if (street != null && street2 == null) {
        fullStreet = street;
      } else if (street == null && street2 != null) {
        fullStreet = street2;
      }

      String? placeId = response.response!.data['results'][0]['place_id'];

      String formattedAddress =
          response.response!.data['results'][0]['formatted_address'].toString();

      _address = [fullStreet, town, formattedAddress, placeId, country];
    } else {
