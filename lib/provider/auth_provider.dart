...
class AuthProvider with ChangeNotifier {
  ...
  Future registration(RegisterModel register, Function callback) async {
    ...
    if (apiResponse.response != null &&
      ...
      callback(true, message, register.phone, register.password);
      notifyListeners();
    } else {
      String? errorMessage;
      if (apiResponse.error is String) {
      ...
      callback(false, errorMessage, register.phone, register.password);
      notifyListeners();
    }
  }
  ...
  Future login(LoginModel loginBody, Function callback) async {
    ...
    if (apiResponse.response != null &&
        apiResponse.response!.statusCode == 200) {
      SignInResult signInResult = apiResponse.response!.data;
      ...
      if (signInResult != null) {
        authRepo!.saveUserLogin(signInResult.isSignedIn);
        await authRepo?.updateToken(loginBody.phone);
      }

      callback(true, signInResult.isSignedIn, "noError");
      notifyListeners();
    } else {
      String? errorMessage;
      if (apiResponse.error is String) {
  ...
  Future<void> updateToken(String? userPhone, BuildContext context) async {
    ApiResponse apiResponse = await authRepo!.updateToken(userPhone);
    if (apiResponse.response != null &&
        apiResponse.response?.statusCode == 200) {
    } else {
  ...
  Future<ResponseModel> resendCode(String phone) async {
    _isPhoneNumberVerificationButtonLoading = true;
    _verificationMsg = '';
    notifyListeners();
    ApiResponse apiResponse = await authRepo!.resendCode(phone);
    _isPhoneNumberVerificationButtonLoading = false;
    notifyListeners();
    ResponseModel responseModel;
    if (apiResponse.response != null &&
        apiResponse.response!.statusCode == 200) {
      responseModel = ResponseModel(apiResponse.response!.data, true);
    } else {
      String? errorMessage;
      if (apiResponse.error is String) {
        print(apiResponse.error.toString());
  ...
  Future<ResponseModel> verifyOtp(String phone) async {
    ...
    if (apiResponse.response != null &&
        apiResponse.response!.statusCode == 200) {
      responseModel = ResponseModel(
          apiResponse.response!.data.isSignUpComplete.toString(), true);
    } else {
      String? errorMessage;
      if (apiResponse.error is String) {
        print(apiResponse.error.toString());
  ...
  Future<ResponseModel> resetPassword(String identity, String otp,
    ...
    if (apiResponse.response != null &&
      ...
      responseModel = ResponseModel("Password reset successfully", true);
    } else {
      String? errorMessage;
      if (apiResponse.error is String) {
        print(apiResponse.error.toString());
  ...
  Future<ResponseModel> forgetPassword(String email) async {
    ...
    if (apiResponse.response != null &&
        apiResponse.response!.statusCode == 200) {
      responseModel = ResponseModel(
        apiResponse.response!.data.isPasswordReset.toString(),
        true,
      );
    } else {
      String? errorMessage;
